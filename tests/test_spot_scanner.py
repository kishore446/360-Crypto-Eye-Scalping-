...
class TestSpotScannerScamDetection:
    ...
    def test_no_scam_on_normal_candles(self):
        ...
        base = 1.0
        for i in range(24):
            t = int(time.time() * 1000) - (24 - i) * 3600_000
            vol = base * (1000 + random.uniform(-200, 200))  # variable volume
            c = [t, base, base * 1.01, base * 0.99, base * (1 + random.uniform(-0.01, 0.01)), vol]
            candles.append(c)

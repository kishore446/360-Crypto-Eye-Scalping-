...
class TestTradeResultFields:
    def test_default_channel_tier(self):
        ...
        assert r.channel_tier == "CH1_SCALPING"
    ...
    def test_to_dict_includes_channel_tier(self):
        r = _make_result(channel_tier="CH2_INTRADAY", session="OVERLAP")
        d = r.to_dict()
        assert d["channel_tier"] == "CH2_INTRADAY"
        assert d["session"] == "OVERLAP"

    def test_from_dict_round_trips(self):
        r = _make_result(channel_tier="CH3_TREND", session="ASIA")
        r2 = TradeResult.from_dict(r.to_dict())
        assert r2.channel_tier == "CH3_TREND"
...
class TestPerChannelStats:
    def test_empty_dashboard(self, db):
        ...
        assert "CH1_SCALPING" in stats
        assert stats["CH1_SCALPING"]["total_signals"] == 0
        assert stats["CH1_SCALPING"]["win_rate"] == 0.0

    def test_single_win(self, db):
        db.record_result(_make_result(channel_tier="CH1_SCALPING", outcome="WIN", pnl_pct=2.0))
        stats = db.per_channel_stats()
        assert stats["CH1_SCALPING"]["total_signals"] == 1
        assert stats["CH1_SCALPING"]["win_rate"] == 100.0
        assert stats["CH1_SCALPING"]["wins"] == 1

    def test_multiple_channels(self, db):
        db.record_result(_make_result(channel_tier="CH1_SCALPING", outcome="WIN", pnl_pct=3.0))
        db.record_result(_make_result(channel_tier="CH1_SCALPING", outcome="LOSS", pnl_pct=-1.0))
        db.record_result(_make_result(channel_tier="CH2_INTRADAY", outcome="WIN", pnl_pct=1.5))
        stats = db.per_channel_stats()
        assert stats["CH1_SCALPING"]["total_signals"] == 2
        assert stats["CH1_SCALPING"]["win_rate"] == 50.0
        assert stats["CH2_INTRADAY"]["total_signals"] == 1
        assert stats["CH2_INTRADAY"]["win_rate"] == 100.0

    def test_all_tiers_present(self, db):
        stats = db.per_channel_stats()
        for tier in ("CH1_SCALPING", "CH2_INTRADAY", "CH3_TREND", "CH4_SPOT", "AGGREGATE"):
            assert tier in stats

    def test_best_and_worst_trade(self, db):
        db.record_result(_make_result(channel_tier="CH1_SCALPING", outcome="WIN", pnl_pct=5.0))
        db.record_result(_make_result(channel_tier="CH1_SCALPING", outcome="LOSS", pnl_pct=-2.0))
        stats = db.per_channel_stats()
        assert stats["CH1_SCALPING"]["best_trade"] == 5.0
        assert stats["CH1_SCALPING"]["worst_trade"] == -2.0

    def test_open_trades_excluded(self, db):
        db.record_result(_make_result(channel_tier="CH1_SCALPING", outcome="OPEN", pnl_pct=1.0))
        stats = db.per_channel_stats()
        assert stats["CH1_SCALPING"]["total_signals"] == 0
...
class TestFormatPerChannelReport:
    def test_report_contains_channel_labels(self, db):
        ...
        assert "CH1 Scalping" in report
        assert "CH2 Intraday" in report
        assert "CH3 Trend" in report
    ...
    def test_report_with_data(self, db):
        ...
        for _ in range(3):
            db.record_result(
                _make_result(
                    channel_tier="CH1_SCALPING", outcome="WIN", pnl_pct=2.0,
                    opened_at=now - 86400,
                )
            )
        db.record_result(
            _make_result(
                channel_tier="CH1_SCALPING", outcome="LOSS", pnl_pct=-1.0,
                opened_at=now - 86400,
            )
        )
    ...
    def test_report_respects_days_window(self, db):
        ...
        db.record_result(
            _make_result(channel_tier="CH1_SCALPING", outcome="WIN", pnl_pct=5.0, opened_at=old_ts)
        )
        report = db.format_per_channel_report(days=30)
...
class TestRollingWinRateUsesClosedAt:
    ...
    def test_win_rate_rolling_uses_closed_at(self, db):
        ...
        r = _make_result(channel_tier="CH1_SCALPING", outcome="WIN", pnl_pct=2.0, opened_at=now - 10 * 86400)
    ...
    def test_win_rate_rolling_excludes_trade_closed_outside_window(self, db):
        now = time.time()
        r = _make_result(channel_tier="CH1_SCALPING", outcome="WIN", pnl_pct=2.0, opened_at=now - 3 * 86400)
    ...
    def test_format_per_channel_report_uses_closed_at(self, db):
        ...
        r = _make_result(
            channel_tier="CH1_SCALPING", outcome="WIN", pnl_pct=3.0,
...
class TestStaleTracking:
    ...
    def test_stale_count_in_channel_stats(self, db):
        db.record_result(_make_result(channel_tier="CH1_SCALPING", outcome="STALE", pnl_pct=0.0))
        db.record_result(_make_result(channel_tier="CH1_SCALPING", outcome="WIN", pnl_pct=2.0))
        stats = db.per_channel_stats()
        assert stats["CH1_SCALPING"]["stale_count"] == 1
...
class TestPerChannelRollingStats:
    ...
    def test_rolling_includes_recent_trades(self, db):
        now = time.time()
        r = _make_result(channel_tier="CH1_SCALPING", outcome="WIN", pnl_pct=2.0,
...
class TestPerChannelProfitFactor:
    def test_empty_returns_zero(self, db):
        ...
        assert pf["CH1_SCALPING"] == pytest.approx(0.0)

    def test_all_wins_returns_inf_no_loss(self, db):
        """When there are only winning trades, profit factor is infinite (perfect record)."""
        db.record_result(_make_result(channel_tier="CH1_SCALPING", outcome="WIN", pnl_pct=2.0))
        pf = db.per_channel_profit_factor()
        assert math.isinf(pf["CH1_SCALPING"])

    def test_wins_and_losses(self, db):
        db.record_result(_make_result(channel_tier="CH1_SCALPING", outcome="WIN", pnl_pct=4.0))
        db.record_result(_make_result(channel_tier="CH1_SCALPING", outcome="LOSS", pnl_pct=-2.0))
        pf = db.per_channel_profit_factor()
        assert pf["CH1_SCALPING"] == pytest.approx(2.0)


class TestPerChannelTpDistribution:
    def test_empty_returns_zeros(self, db):
        dist = db.per_channel_tp_distribution()
        assert dist["CH1_SCALPING"]["WIN"] == 0
        assert dist["CH1_SCALPING"]["LOSS"] == 0
        assert dist["CH1_SCALPING"]["STALE"] == 0

    def test_distribution_counts(self, db):
        db.record_result(_make_result(channel_tier="CH1_SCALPING", outcome="WIN", pnl_pct=2.0))
        db.record_result(_make_result(channel_tier="CH1_SCALPING", outcome="WIN", pnl_pct=3.0))
        db.record_result(_make_result(channel_tier="CH1_SCALPING", outcome="LOSS", pnl_pct=-1.0))
        db.record_result(_make_result(channel_tier="CH1_SCALPING", outcome="BE", pnl_pct=0.0))
        db.record_result(_make_result(channel_tier="CH1_SCALPING", outcome="STALE", pnl_pct=0.0))
        dist = db.per_channel_tp_distribution()
        assert dist["CH1_SCALPING"]["WIN"] == 2
        assert dist["CH1_SCALPING"]["LOSS"] == 1
        assert dist["CH1_SCALPING"]["BE"] == 1
        assert dist["CH1_SCALPING"]["STALE"] == 1
        assert dist["CH1_SCALPING"]["total"] == 5


class TestPerChannelEquityCurve:
    def test_empty_curve(self, db):
        curves = db.per_channel_equity_curve()
        assert curves["CH1_SCALPING"] == []

    def test_curve_cumulative(self, db):
        now = time.time()
        r1 = _make_result(channel_tier="CH1_SCALPING", outcome="WIN", pnl_pct=2.0, opened_at=now - 2 * 3600)
        r1 = TradeResult(**{**r1.to_dict(), "closed_at": now - 2 * 3600})
        r2 = _make_result(channel_tier="CH1_SCALPING", outcome="WIN", pnl_pct=3.0, opened_at=now - 1 * 3600)
        r2 = TradeResult(**{**r2.to_dict(), "closed_at": now - 1 * 3600})
        db.record_result(r1)
        db.record_result(r2)
        curves = db.per_channel_equity_curve()
        assert len(curves["CH1_SCALPING"]) == 2
        assert curves["CH1_SCALPING"][0] == pytest.approx(2.0)
        assert curves["CH1_SCALPING"][1] == pytest.approx(5.0)

...
from bot.ws_manager import _WS_FUTURES_URL, _WS_SPOT_URL, MarketDataStore, WebSocketManager
...
class TestMarketDataStoreBuffers:
    ...
    def test_spot_has_sufficient_data_checks_1h(self):
        ...
        for i in range(3):
            store.update_candle("ETH", "1h", [i * 3_600_000, 3000, 3100, 2900, 3050, 100])
        for i in range(2):
            store.update_candle("ETH", "4h", [i * 14_400_000, 3000, 3100, 2900, 3050, 100])
        for i in range(20):
            store.update_candle("ETH", "1d", [i * 86_400_000, 3000, 3100, 2900, 3050, 100])

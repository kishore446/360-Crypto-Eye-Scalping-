...
Three channel tiers with differentiated gate requirements:

  CH1 (Hard Scalp):    7 gates mandatory, score ≥ 70/100
  CH2 (Medium Scalp):  5 gates mandatory, score ≥ 50/100
  CH3 (Easy Breakout): 3 gates + momentum, score ≥ 35/100

Gates:
  ① Macro Bias (1D + 4H alignment)
  ② Discount/Premium Zone
  ③ Liquidity Sweep (stop-hunt)
  ④ MSS/ChoCh with volume confirmation
  ⑤ Order Block on 15m
  ⑥ Fair Value Gap on 15m
  ⑦ News filter (60m/30m/15m per channel)
  ⑧ Funding Rate adjustment (±5 score)
  ⑨ OI Divergence adjustment (±5 score)
"""
...
def run_confluence_check_ch1_hard(
    ...
    try:
        from config import (
            ...
            CH1_TP1_RR, CH1_TP2_RR, CH1_TP3_RR,
            CH1_MIN_CONFLUENCE,
        )
    except ImportError:
...
def run_confluence_check_ch2_medium(
    ...
    try:
        from config import (
            CH2_LEVERAGE_MIN, CH2_LEVERAGE_MAX,
            CH2_TP1_RR, CH2_TP2_RR, CH2_TP3_RR,
            CH2_MIN_CONFLUENCE, CH2_NEWS_WINDOW_MINUTES,
        )
    except ImportError:
...
def run_confluence_check_ch3_easy(
    ...
    try:
        from config import (
            ...
            CH3_TP1_RR, CH3_TP2_RR, CH3_TP3_RR,
            CH3_MIN_CONFLUENCE,
        )
    except ImportError:
    ...
    if funding_rate is not None:
        ...
        if side == Side.LONG:
            if funding_rate > funding_hard_positive:
                logger.info(
                    "[GATE_FAIL][CH3] %s %s: gate=funding_rate reason=extreme_long_crowding rate=%.6f",
                    symbol, side.value, funding_rate,
                )
                return None
            elif funding_rate < FUNDING_EXTREME_NEGATIVE:
            ...
            if funding_rate < funding_hard_negative:
                logger.info(
                    "[GATE_FAIL][CH3] %s %s: gate=funding_rate reason=extreme_short_crowding rate=%.6f",
                    symbol, side.value, funding_rate,
                )
                return None
            elif funding_rate > FUNDING_EXTREME_POSITIVE:
    ...
    return SignalResult(
        ...
        tp3=tp3,
        stop_loss=stop_loss,
        structure_note=structure_note or f"{'Bullish' if side == Side.LONG else 'Bearish'} breakout momentum -- 3-gate easy entry.",

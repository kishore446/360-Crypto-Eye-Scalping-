...
def save_signal(signal_data: dict) -> None:
    ...
    *signal_data* must contain at minimum the fields that map to the columns
    of the ``signals`` table.  Missing optional fields default to ``None`` /
...
def load_active_signals() -> list[dict]:
    ...
    -------
    list[dict]
        Each dict has keys matching the ``signals`` table columns, with
        ``sqlite3.Row`` converted to a plain ``dict``.
    """
    with _get_conn() as conn:
        rows = conn.execute(
